...
namespace MovieShop.MVC.App_Start
{
    ...
    public static class NinjectWebCommon 
    {
        ...
        private static void RegisterServices(IKernel kernel) // .net does not need third party injection  
        {
            kernel.Bind<IMovieService>().To<MovieService>(); 
            //kernel.Bind<IMovieService>().To<MovieServiceTest>();
            kernel.Bind<IGenreService>().To<GenreService>();
            kernel.Bind<IMovieRepository>().To<MovieRepository>();
            kernel.Bind<IGenreRepository>().To<GenreRepository>(); //binding the class to the interface 
        }
    }
}
